...
- `--with-user-reviewers`: merge personal reviewers into a repository's configured reviewer set. This applies only to a purely local review; it is rejected with `--repo`/`--pr` and with `bastion validate FILE`.
