...
`bastion review` applies each reviewer's path trigger or agent-assisted trigger to the changes against the selected base; routing, the runner, and aggregation are the shared core, not local-specific. An explicit `--base` always wins. Without one, Bastion runs `gh pr view` to detect the PR for the current branch and uses its direct base commit. If no PR is detected, or `gh` cannot run, it uses `main`. If `gh` runs and fails, it prints a warning and uses the same fallback. The live working tree remains under review, including uncommitted and untracked files. Native GitHub stacks therefore review one layer at a time: C against B, B against A, and A against trunk.

Two other things differ from CI. The first is reviewer discovery: a purely local run uses the user-level registry as a fallback when no repository configuration exists, and `--with-user-reviewers` merges both registries. A GitHub-source run uses the repository's reviewers alone (see [User-level reviewers](#user-level-reviewers)). The second is where inputs come from: there is no preview-deploy job, so anything a reviewer's `env` or `inputs` reference is expected to be in the local environment already. A `precommit` script might boot the service on `http://localhost:3000` and export that as the preview URL. A native reviewer inherits that local environment directly. A containerized reviewer (one with a `runner` and `capabilities.network: true`) inherits none of it; only its literal `env` pairs and the fixed provider-credential set cross into the container, so a local value reaches it only when written into the reviewer's `env` (see [Containers](./containers.md)).
...
The [review context](./design.md#review-context) uses the detected PR body as intent (or the title when the body is empty). Without a detected PR, intent comes from the branch's commit messages (`base..HEAD`). Automatic local detection also gathers discussion through `gh api`, the same comments CI reads. If there is no PR, or `gh` cannot run, the review continues without that context. If `gh` runs and fails, Bastion warns and continues the same way. Prior-findings memory works because every local run is persisted: a second `bastion review` on the same branch shows each reviewer what it raised last time, recalled from the run store.
