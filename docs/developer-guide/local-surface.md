...
The [review context](./design.md#review-context) uses the detected PR body as intent. Without a detected PR, intent comes from the branch's commit messages (`base..HEAD`). Automatic local detection does not gather discussion; an explicit `--repo`/`--pr` run can add it through the REST client. Prior-findings memory works because every local run is persisted: a second `bastion review` on the same branch shows each reviewer what it raised last time, recalled from the run store.
