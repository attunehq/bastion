...
| [`src/akari.rs`](../../src/akari.rs) | Opt-in handoff of native agent sessions to the installed Akari client. Disabled by default; enablement is user-level (`akari.yaml` or `BASTION_AKARI`), never repository `.bastion.yaml`. Isolates session files under the data dir and shells out to `akari ingest`; a handoff failure does not change a verdict. |
...
3. **Select and compute the changeset** (`git.rs`, `github/context.rs`). An
