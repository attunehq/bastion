...
| `run.attestation-fallback` | An attestation was *offered but refused*; carries the `reason` (an unreadable or unverifiable note, an unregistered key, a stale binding, a dirty checkout, and so on). A commit that offered no note at all is not a refusal and emits no such event: it runs fresh silently. |
