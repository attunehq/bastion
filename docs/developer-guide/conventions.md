...
- **A backend that cannot verdict reviews nothing.** All five backends
  (`claude-code`, `codex`, `pi`, `grok`, `muse`) are wired, but any of them returns
  an error rather than fabricating a pass when it cannot produce a valid,
  consistent verdict; the runner turns that into a fail-closed block for a gate. See
