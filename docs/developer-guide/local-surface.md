...
The loop's dominant cost is re-executing reviewers that already passed: after fixing one reviewer's findings, a naive re-run re-executes the whole triggered set even though most of it judged content the fix never touched. So a purely local `bastion review` is incremental by default. Every resolved reviewer is stamped with a *scope digest*: a hash of the reviewer's own effective definition plus the diff of exactly the changed files its trigger matched (working tree against the merge base; untracked matched files hashed by content). On the next run of the same branch, a reviewer whose prior verdict was a **pass** and whose digest is unchanged is *carried*: its prior verdict folds into the run (`"carried": true` on `reviewer.resolved`, no usage, zero duration) without a backend executing, and it still counts in the gate tally. A reviewer whose scoped content changed, which always includes the ones that blocked (the fix touched the files they flagged), executes fresh. Blocks are never carried: re-confirming a block is exactly the loop's next question.
...
Three guards keep carry honest:
